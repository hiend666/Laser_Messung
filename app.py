...
def _on_v_time_base_cb():
    # Der Widget-Wert (v_time_base_s_sw) liegt in Slider-Anzeigeeinheit vor (z.B. ms).
    # Er wird hier UNVERÄNDERT als Zwischenspeicher für den nächsten Rerun abgelegt.
    # Die Umrechnung in Sekunden erfolgt im Hauptfluss via _tb_hz.
    # float64 mit voller Auflösung (~15 Stellen) – mind. 9 Nachkommastellen
    # für µs-Bereich bei kleinen Crops (z.B. 0.000005 s = 5 µs).
    st.session_state['v_time_base_s'] = float(np.float64(st.session_state['v_time_base_s_sw']))
